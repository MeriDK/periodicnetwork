...
for ds in ("asassn", "macho", "ogle3"):
    ...
    for seed in range(8):
        ...
        train_index, test_index = joblib.load(
            root / "seeds" / f"{ds}_{seed}" / "index.pkl"
        )
        ...
        train_split = [
            ...
            for chunk in data[i].split(L, L)
            if data[i].label is not None
        ]
        test = [
            chunk
            for i in test_index
        ...
        schema = {
            "mjd": pl.List(pl.Float32),
            "mag": pl.List(pl.Float32),
            "magerr": pl.List(pl.Float32),
            "period": pl.Float32,
            "label": pl.Utf8,
        }
